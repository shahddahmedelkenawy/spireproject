...
/** One saved preset per signed-in user (localStorage, this device) */
const USER_KEY_PREFIX = 'spire-accessibility-v4-uid-'
...
export const useAccessibilityStore = defineStore('accessibility', () => {
  ...
  /** If set, preference changes are persisted under the signed-in user; if null, session uses defaults and changes are not saved */
  const currentAuthUid = ref(null)
  /** >0 while batching ref writes so the settings watch does not persist or full apply per field */
  ...
  function persist() {
    const uid = currentAuthUid.value
    if (!uid) return
    try {
      localStorage.setItem(userStorageKey(uid), JSON.stringify(getState()))
    } catch {
      /* ignore */
    }
  }
  ...
  function applyPayloadToRefs(p, migratedFromV3Only) {
  ...
  /**
   * If this user has no file yet, copy legacy app-wide v4 / v3 keys into a per-user key and remove the globals.
   * Returns the JSON string to parse and the v3 migration flag, or { raw: null } if nothing to do.
   */
  function readUserPayloadOrMigrate(uid) {
    const k = userStorageKey(uid)
    let raw = null
    try {
      raw = localStorage.getItem(k)
    } catch {
      return { raw: null, migratedFromV3Only: false, emptyUserKey: !raw }
    }
    if (raw) return { raw, migratedFromV3Only: false, emptyUserKey: false }
    const rawV4 = localStorage.getItem(STORAGE_KEY)
    const rawV3 = localStorage.getItem(STORAGE_KEY_V3)
    if (rawV4) {
      try {
        localStorage.setItem(k, rawV4)
        localStorage.removeItem(STORAGE_KEY)
        return { raw: rawV4, migratedFromV3Only: false, emptyUserKey: true }
      } catch {
        /* fall through to v3 */
      }
    }
    if (rawV3) {
      try {
        localStorage.setItem(k, rawV3)
        localStorage.removeItem(STORAGE_KEY_V3)
        return { raw: rawV3, migratedFromV3Only: true, emptyUserKey: true }
      } catch {
        /* ignore */
      }
    }
    return { raw: null, migratedFromV3Only: false, emptyUserKey: true }
  }

  function applyV2IfPresent() {
  ...
  /**
   * Call when auth uid changes. Not signed in: in-memory defaults (guest session) and do not read/write localStorage.
   * Signed in: load this user's saved preferences (migrates legacy app-wide key once on first login).
   */
  function setUserContext(uid) {
    flushPersist()
    hydrationDepth += 1
    try {
      const nextUid = typeof uid === 'string' && uid.length > 0 ? uid : null
      currentAuthUid.value = nextUid
      if (!nextUid) {
        applyDefaultRefs()
        stopSpeechToText()
        ...
        const { raw, migratedFromV3Only, emptyUserKey } = readUserPayloadOrMigrate(nextUid)
        if (raw) {
          try {
            const p = JSON.parse(raw)
            applyPayloadToRefs(p, migratedFromV3Only)
          } catch {
            applyDefaultRefs()
          }
        } else {
    ...
    applyToDOM(getState())
    if (currentAuthUid.value) {
      try {
        localStorage.setItem(userStorageKey(currentAuthUid.value), JSON.stringify(getState()))
      } catch {
        /* ignore */
      }
    }
